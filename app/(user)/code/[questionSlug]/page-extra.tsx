...
import { ChevronLeft, Play, Check, LightbulbIcon, Maximize, Minimize, PanelLeftClose, PanelLeftOpen } from "lucide-react";
import Editor from '@monaco-editor/react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useRouter, useSearchParams } from "next/navigation";
import Link from "next/link";
import { executeCode } from '@/lib/actions/code';
import FullScreenCodeModal from '@/components/ui/full-screen-code-modal';
...
// Map editor language → Judge0 IDs
...
function PracticePageContent() {
    ...
    const submitCode = async () => {
        ...
        const promises = selectedProblem.examples.map(async (example) => {
            try {
                const data = await executeCode(
                    code,
                    judge0Langs[language],
                ...
                if (actual === expected) {
                    return `✅ Passed | Input: ${example.input}`;
                } else {
                    return `❌ Failed | Input: ${example.input}\nExpected: ${expected}\nGot: ${actual}`;
                }
            } catch (err: any) {
                ...
                <div className="flex-1 flex flex-col overflow-hidden relative">
                    <div className="flex flex-1 overflow-hidden">
                        ...
                        <div className="w-1/3 border-r border-gray-200 dark:border-gray-800 flex flex-col">
                            <div className="border-b border-gray-200 dark:border-gray-800 p-3 flex gap-2">
                                {!isPanelOpen && (
                                    <Button
                                        variant="outline"
                                        size="sm"
                                ...
                                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                                    ...
                                    <TabsContent value="description" className="space-y-4">
                                        ...
                                        <ConstraintsList constraints={selectedProblem.constraints} />
                                        {(showHint && selectedProblem.hint) && <HintBox hint={selectedProblem.hint} />}
                                    </TabsContent>
                        ...
                        <div className="w-2/3 flex flex-col overflow-hidden">
                            ...
                            <EditorActions runCode={runCode} submitCode={submitCode} isRunning={isRunning} />
                        </div>
                    </div>
                </div>
            </div>
...
function ExampleCard({ example, index }: any) {
    return (
        <div className="p-4 border rounded-md">
            <h3 className="text-sm font-semibold mb-2">Sample {index + 1}:</h3>
            <div className="grid grid-cols-1 gap-3">
                <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Input:</p>
                    <pre className="text-sm font-mono whitespace-pre-wrap">{example.input}</pre>
